 ...
 * password server-side, flags the session, then hard-navigates to the points
 * home. The full reload is deliberate: it guarantees the fetch interceptor is
 * installed from main.jsx before any component mounts and fires its first
 * request, instead of racing a client-side route change.
 */
import React, { useEffect, useState } from 'react';

const ORANGE = '#FF5500';

export default function PointsDemoGate() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch('/api/demo-access', { credentials: 'same-origin' })
  ...
  async function enterDemo() {
    const { markDemoActive, markLiveSeedDemo } = await import('./installDemoBackend.js');
    markDemoActive();
    // The second flag is what separates this from the video demo: it selects
    // the live backend snapshot over the invented markets, and namespaces the
    // saved scenario so the two demos never inherit each other's board.
    markLiveSeedDemo();
        ...
        {state === 'unlocked' ? (
          <>
            <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>
              Ya tienes acceso. Adentro verás los mercados reales de Pronos con
              actividad simulada — nada de lo que hagas toca la base de datos.
            </p>
            <button
              type="button"
              onClick={enterDemo}
