...
import { normalizeInterestPayload } from './_lib/interest.js';
...
function visitorKey(req, res) {
  ...
  const raw = cookies[COOKIE_NAME] || crypto.randomUUID();
...
export default async function handler(req, res) {
  ...
  let payload;
  try {
    payload = normalizeInterestPayload(requestBody(req));
  } catch (e) {
  ...
  try {
    ...
    const uniqueRows = await sql`
      INSERT INTO interest_daily_visitors (
        ...
        ${visitorKey(req, res)}, ${payload.surface}, ${payload.objectType}, ${payload.objectId}, ${payload.action}, CURRENT_DATE
      )
      ON CONFLICT DO NOTHING
      RETURNING 1
    `;
    const uniqueInc = uniqueRows.length > 0 ? 1 : 0;
    ...
    await sql`
      INSERT INTO interest_daily_counts (
        ...
        1, ${uniqueInc}, ${JSON.stringify(payload.metadata)}::jsonb, NOW(), NOW()
      )
      ON CONFLICT (surface, object_type, object_id, action, day)
      DO UPDATE SET
        count = interest_daily_counts.count + 1,
        unique_count = interest_daily_counts.unique_count + ${uniqueInc},
        metadata = interest_daily_counts.metadata || EXCLUDED.metadata,
        last_seen_at = NOW()
    `;

    return res.status(200).json({ ok: true, unique: uniqueInc > 0 });
  } catch (e) {
