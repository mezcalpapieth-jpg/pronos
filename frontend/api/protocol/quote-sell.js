/**
 * POST /api/protocol/quote-sell
 * Body: { marketId, outcomeIndex, shares }
 *
 * Read-only on-chain AMM sell quote. Used to produce a
 * minCollateralOut guard before sending /api/protocol/sell.
 */
import { neon } from '@neondatabase/serverless';
import { applyCors } from '../_lib/cors.js';
import { rateLimit, clientIp } from '../_lib/rate-limit.js';
import { quoteSellOnChain } from '../_lib/onchain-trader.js';
...
export default async function handler(req, res) {
  ...
  const limited = rateLimit(req, res, {
    key: `protocol-quote-sell:${clientIp(req)}`,
    limit: 60,
    windowMs: 60_000,
  });
  if (limited) return;
  ...
  try {
    ...
    console.error('[protocol/quote-sell] error', { message: e?.message, code: e?.code });
