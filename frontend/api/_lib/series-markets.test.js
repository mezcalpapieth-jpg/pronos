...
test('buildSeriesDetail locks approved optional best-of-seven games until the score requires them', () => {
  const meta = {
    ...
    winTarget: 4,
    guaranteedGames: 4,
    round: 'West First Round',
    seasonYear: 2026,
    homeTeam: { id: '25', name: 'Oklahoma City Thunder', shortName: 'Thunder', abbreviation: 'OKC' },
    awayTeam: { id: '24', name: 'San Antonio Spurs', shortName: 'Spurs', abbreviation: 'SAS' },
    teams: [
      { id: '25', name: 'Oklahoma City Thunder', shortName: 'Thunder', abbreviation: 'OKC' },
      { id: '24', name: 'San Antonio Spurs', shortName: 'Spurs', abbreviation: 'SAS' },
    ],
  };
  const outcomes = ['Oklahoma City Thunder', 'San Antonio Spurs'];
  const markets = [
    { id: 301, status: 'resolved', outcome: 0, outcomes, startTime: '2026-05-01T00:00:00Z', seriesMeta: { ...meta, gameNumber: 1 } },
    { id: 302, status: 'resolved', outcome: 0, outcomes, startTime: '2026-05-03T00:00:00Z', seriesMeta: { ...meta, gameNumber: 2 } },
    { id: 303, status: 'active', outcome: null, outcomes, startTime: '2026-05-05T00:00:00Z', seriesMeta: { ...meta, gameNumber: 3 } },
    { id: 304, status: 'active', outcome: null, outcomes, startTime: '2026-05-07T00:00:00Z', seriesMeta: { ...meta, gameNumber: 4 } },
    { id: 305, status: 'active', outcome: null, outcomes, startTime: '2026-05-09T00:00:00Z', seriesMeta: { ...meta, gameNumber: 5 } },
    { id: 306, status: 'active', outcome: null, outcomes, startTime: '2026-05-11T00:00:00Z', seriesMeta: { ...meta, gameNumber: 6 } },
  ];
...
test('buildSeriesDetail unlocks optional best-of-seven games only at their score thresholds', () => {
  const meta = {
    key: 'basketball-nba:2026:west-first-round:24-25',
    leaguePath: 'basketball/nba',
    league: 'nba',
  ...
  const optionalMarkets = [
    { id: 405, status: 'active', outcome: null, outcomes, startTime: '2026-05-09T00:00:00Z', seriesMeta: { ...meta, gameNumber: 5 } },
    { id: 406, status: 'active', outcome: null, outcomes, startTime: '2026-05-11T00:00:00Z', seriesMeta: { ...meta, gameNumber: 6 } },
    { id: 407, status: 'active', outcome: null, outcomes, startTime: '2026-05-13T00:00:00Z', seriesMeta: { ...meta, gameNumber: 7 } },
  ];

  const game5Detail = buildSeriesDetail({ ...meta, espnSeriesWins: { homeWins: 1, awayWins: 1 } }, optionalMarkets);
  assert.deepEqual(game5Detail.sequence.slice(4).map(g => [g.gameNumber, g.status, g.seriesLocked === true]), [
    [5, 'active', false],
    [6, 'pending', true],
    [7, 'pending', true],
  ]);

  const game5ThreeOneDetail = buildSeriesDetail({ ...meta, espnSeriesWins: { homeWins: 3, awayWins: 1 } }, optionalMarkets);
  assert.deepEqual(game5ThreeOneDetail.sequence.slice(4).map(g => [g.gameNumber, g.status, g.seriesLocked === true]), [
    [5, 'active', false],
    [6, 'pending', true],
    [7, 'pending', true],
  ]);

  const game6Detail = buildSeriesDetail({ ...meta, espnSeriesWins: { homeWins: 3, awayWins: 2 } }, optionalMarkets);
  assert.deepEqual(game6Detail.sequence.slice(4).map(g => [g.gameNumber, g.status, g.seriesLocked === true]), [
    [5, 'active', false],
    [6, 'active', false],
    [7, 'pending', true],
  ]);

  const game7Detail = buildSeriesDetail({ ...meta, espnSeriesWins: { homeWins: 3, awayWins: 3 } }, optionalMarkets);
  assert.deepEqual(game7Detail.sequence.slice(4).map(g => [g.gameNumber, g.status, g.seriesLocked === true]), [
    [5, 'active', false],
    [6, 'active', false],
    [7, 'active', false],
  ]);
...
test('seriesTradeLockFromRows blocks direct buys on locked optional games', () => {
  const current = {
    id: 506,
    question: 'Game 6',
  ...
  const rows = [
    {
      ...
      resolver_config: { ...current.resolver_config, series: { ...current.resolver_config.series, gameNumber: 1 } },
    },
    {
      id: 502,
      outcomes: current.outcomes,
      ...
      resolver_config: { ...current.resolver_config, series: { ...current.resolver_config.series, gameNumber: 2 } },
    },
    current,
  ];

  const locked = seriesTradeLockFromRows(current, rows);
...
test('applySeriesGateToMarket keeps optional listed games pending when no score is available yet', () => {
  const market = applySeriesGateToMarket({
    id: 606,
    status: 'active',
    live: true,
    seriesMeta: {
  ...
  assert.equal(market.status, 'pending');
  assert.equal(market.actualStatus, 'active');
  assert.equal(market.live, false);
  assert.equal(market.seriesLocked, true);
});
