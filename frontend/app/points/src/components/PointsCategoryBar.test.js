...
test('points category bar puts new markets before Mexico, Infrastructure, then sports', () => {
  ...
  assert.match(source, /points\.cat\.infraestructura/);
  assert.ok(newMarketsIndex < mexicoIndex, 'Nuevos mercados should stay before Mexico & Latam');
  assert.ok(mexicoIndex < infrastructureIndex, 'Mexico & Latam should appear before Infrastructure');
  assert.ok(infrastructureIndex < sportsIndex, 'Infrastructure should appear before Deportes');
});
